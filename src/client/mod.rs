...
impl Client {
    ...
    fn get_bilibili_id<'b>(r: &Regex, path: &'b str) -> Option<&'b str> {
        ...
        let mut handle = rt.spawn(async move {
            ...
            'wait: loop {
                ...
                'event: loop {
                    ...
                    // Extract Bilibili ID if it exists
                    let vid = match Self::get_bilibili_id(&config.bilibili_regex, &path) {
                        Some(vid) => vid,
                        None => continue 'wait, // Wait for a new event
                    };

                    log::trace!("Fetching segments for {vid}");

                    let seg = select! {
                        // Fetch data
                        s = sponsorblock.fetch(vid.into(), config.categories.clone(), config.action_types.clone()) => {
                            s.ok()
                        },
    ...
    fn exec_loop(&mut self, tx: async_channel::Sender<WorkerEvent>) {
        loop {
            ...
            if let Err(e) = result {
                log::error!(
                    "Unhandled error on plugin bilibili SponsorBlock [{}]: {}",
                    self.name(),
                    e
                );
            }
